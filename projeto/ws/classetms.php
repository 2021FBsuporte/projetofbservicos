...
//---------------------------------------BLOCO COM FUNÇÃO BUSCAR-------------------------------------------- 
        public function buscarTms($id)
        {
            $res = array();
            $cmd = $this->pdo->prepare("SELECT * FROM tms WHERE id = :id");
            $cmd->bindValue(":id",$id);
            $cmd->execute();
...
//---------------------------------------BLOCO COM FUNÇÃO ATUALIZAR-----------------------------------------
        public function atualizarTms($id, $nome, $tms, $nota)
        {
                $cmd = $this->pdo->prepare("UPDATE tms SET nome = :n, tms = :t, nota = :o WHERE id = :id");
                $cmd->bindValue(":n", $nome);
                $cmd->bindValue(":t", $tms);
                $cmd->bindValue(":o", $nota);
                $cmd->bindValue(":id", $id);
                $cmd->execute();
                return true;
            }
        }
//------------------------------------------FIM DO BLOCO ATULIZAR-------------------------------------------        
?>